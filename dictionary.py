...
class Dictionary():
    ...
    def save(self, dictionary: dict, name_file: str):
        'Salva um dicionario em formato TXT'
        try:
            # abrir o arquivo para gravação - o "b" significa que o arquivo é binário
            file_ = open(self.PATH + name_file + '.txt', 'wb')
        except:
            return False
        else:
    ...
    def get(self, name_file: str):
        ...
        try:
            ...
            file_ = open(self.PATH + name_file + '.txt', 'rb')
        except:
            return False
        else:
